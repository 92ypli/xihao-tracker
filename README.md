...
「哪款游戏属于哪家公司」是知识，不是数据，免费接口里没有。但**一旦你给了 TapTap 链接，口碑就能自动抓**——评分、好评率、评价数、上线日期、甚至「在测」「先行服」这种状态标签，都在 TapTap 的公开接口里。

所以 `config/profiles.json` 的设计是：你填产品名和 TapTap 链接，剩下的交给我。
      ...
      "products": [
        {
          ...
          "launchDate": "2022-10",
          "taptap": "142793",
          "note": "赛季制刷宝 ARPG，核心玩家口碑不错，付费设计有争议"
        }
      ]
    }
  }
...
**TaPpTap 链接怎么拿：** 在 TapTap 里打开产品页，地址栏 `/app/` 后面那串数字就是。`taptap` 字段填 id 或完整链接都行，程序会自己解析。
...
评分是客观数据，**但「口碑」还有一层判断**——比如「付费设计有争议」「前代产品拖累 IP」这种，是从社区讨论里读出来的，不是接口能给的。

我没有替你写这些。原因和产品管线一样：凭印象写出来的评价，你没法验证，还可能已经过时。**填几句你自己确认过的，比四十家公司都写满但内容存疑有价值。**

`profiles.json` 里现在只放了心动公司一家作为示例，其余留白——页面上会显示「这家公司还没有填产品档案」，是个可填的槽位，不是坏了。
