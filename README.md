...
页面顶部有一块「隔夜外盘与风险背景」，取这些指标：

| 指标 | 为什么看它 |
|---|---|
| 纳斯达克 / 标普500 / 道琼斯 | 隔夜科技情绪，影响港股开盘 |
| **恒生科技指数** | **港股游戏股最直接的锚** |
| 恒生指数 / 日经225 / 德国DAX | 亚洲和欧洲风险偏好 |
| VIX 恐慌指数 | 避险情绪（东财没有，走腾讯接口） |
| 美元指数 / 离岸人民币 | 人民币贬 → 外资对港股态度转弱 |
| COMEX 黄金 | 避险资金流向 |
...
```
背景判断：偏谨慎
· 恒生科技跌 -2.04%，港股科技已先行走弱
· VIX 21.67，避险情绪中性
```

### 为什么不做次日股价预测

**因为做出来会害你。** 个股日频收益接近随机游走，基于公开信息的预测模型解释力普遍不到 1%。给出一个具体数字，它的准确率和扔硬币差不多，但你会拿它做决定。

现成的反例就在数据里：同一天、同一板块，恺英网络 +3.51%、三七互娱 -1.14%。这种分化不是任何宏观因子能提前算出来的。

所以这块的定位是**告诉你现在处在什么环境**，不告诉你明天涨跌。规则全部写在 `src/macro.mjs` 里——看得见、改得动、事后能核对对错。**不要拿它当交易依据。**

至于地缘冲突这类事件，没有可靠的结构化数据源。它已经隐含在 VIX 和黄金里了——避险资产的反应本身就是市场对它的定价。
